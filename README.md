...
当前开发分支：`codex/fix-p0-query-filters`
...
- 搜索入口只保留在工单管理、商品服务管理和订单管理；商品服务状态固定为“上架/下架”，订单状态固定为“待服务/已完成/售后中”，其余管理页面不再展示搜索栏。
...
工单查询可额外使用 `personnelId` 和 `customerId`。其余管理页面不显示搜索入口。
...
```text
后端：mvn.cmd -q test 通过（50 项）
前端：npm.cmd test 通过（15 项）
